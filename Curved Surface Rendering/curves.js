...
function computeQBezier(t, points) {
	// Return if there is only is a point
	if (points.length < 2) {
		return points[0];
	}
	...
	// Recursive call
	return computeQBezier(t, resultPts);
}
...
function computeBezier() {
	...
	for (let i = 0; i < nSegment + 1; ++i) {
		// t value
		let t = i / nSegment;
		// Get curve points based on Q(t)
		curvePts.push(computeQBezier(t, controlPts));
	}
	// ---YOUR CODE ENDS HERE---
...
function computeBSpline() {
	...
	// ===YOUR CODE STARTS HERE===
	if (closedCurve) {
		for (let n = 0; n < controlPts.length; ++n) {
			for (let i = 0; i < nSegment + 1; ++i) {
				// Compute t value
				let t = i / nSegment;
				// Precompute t cube
				...
				// Precompute t square
				let tSquare = Math.pow(t, 2);
				// Precompute the matrix
				let tMatrix = [
					(-1 * tCube + 3 * tSquare - 3 * t + 1) / 6,
					(3 * tCube - 6 * tSquare + 4) / 6,
					(-3 * tCube + 3 * tSquare + 3 * t + 1) / 6,
					tCube / 6
				];
				// x-values for the curve point
				let xMatrix = [
					tMatrix[0] * controlPts[n].x,
					tMatrix[1] * controlPts[(n + 1) % controlPts.length].x,
					tMatrix[2] * controlPts[(n + 2) % controlPts.length].x,
					tMatrix[3] * controlPts[(n + 3) % controlPts.length].x
				];
				// y-values for the curve point
				let yMatrix = [
					tMatrix[0] * controlPts[n].y,
					tMatrix[1] * controlPts[(n + 1) % controlPts.length].y,
					tMatrix[2] * controlPts[(n + 2) % controlPts.length].y,
					tMatrix[3] * controlPts[(n + 3) % controlPts.length].y
				];
				// Computing the curve point
				curvePts.push({
					// x value
					x: xMatrix[0] + xMatrix[1] + xMatrix[2] + xMatrix[3],
					// y value
					y: yMatrix[0] + yMatrix[1] + yMatrix[2] + yMatrix[3]
				});
			}
		}
	}
	else {
		for (let n = 0; n < controlPts.length - 3; ++n) {
			for (let i = 0; i < nSegment + 1; ++i) {
				// Compute t value
				let t = i / nSegment;
				// Precompute t cube
				...
				let xMatrix = [
					tMatrix[0] * controlPts[n].x,
					tMatrix[1] * controlPts[n + 1].x,
					tMatrix[2] * controlPts[n + 2].x,
					tMatrix[3] * controlPts[n + 3].x
				];
				// y-values for the curve point
				let yMatrix = [
					tMatrix[0] * controlPts[n].y,
					tMatrix[1] * controlPts[n + 1].y,
					tMatrix[2] * controlPts[n + 2].y,
					tMatrix[3] * controlPts[n + 3].y
				];
				// Computing the curve point
				curvePts.push({
					// x value
	...
	if (closedCurve) {
		...
		for (let i = 0; i < controlPts.length; ++i) {
			for (let n = 0; n < nSegment + 1; ++n) {
				...
				// Compute the x and y independently for curve point
				curvePts.push({
					// Curve point x
					x: tMatrix[0] * controlPts[i].x + tMatrix[1] * controlPts[(i + 1) % controlPts.length].x + tMatrix[2] * 2 * controlPts[i].dx + tMatrix[3] * 2 * controlPts[(i + 1) % controlPts.length].dx,
					// Curve point y
					y: tMatrix[0] * controlPts[i].y + tMatrix[1] * controlPts[(i + 1) % controlPts.length].y + tMatrix[2] * 2 * controlPts[i].dy + tMatrix[3] * 2 * controlPts[(i + 1) % controlPts.length].dy
				});
			}
		}
	}
	else {
		// For not closed curve
		for (let i = 0; i < controlPts.length - 1; ++i) {
			for (let n = 0; n < nSegment + 1; ++n) {
				// Compute t value
				let t = n / nSegment;
				...
				curvePts.push({
					// Curve point x
					x: tMatrix[0] * controlPts[i].x + tMatrix[1] * controlPts[i + 1].x + 2 * tMatrix[2] * controlPts[i].dx + tMatrix[3] * 2 * controlPts[i + 1].dx,
					// Curve point y
					y: tMatrix[0] * controlPts[i].y + tMatrix[1] * controlPts[i + 1].y + 2 * tMatrix[2] * controlPts[i].dy + tMatrix[3] * 2 * controlPts[i + 1].dy
				});
			}
		}
	}
